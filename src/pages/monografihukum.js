...
const Monografihukum = ({ data }) => {
  ...
  function clickDetailEvent(j, t, jdl) {
    ...
    router.push(
      "/detailmonografi/" + noSlashj + "/" + judul2 + "/" + t,
      undefined,
      { shallow: true }
    );
  }
  ...
  const handleClickbtnSearch = (csp) => {
    ...
    router.push(
      "/monografi-hukum/" + lctx[1] + "/" + encodeURI(link),
      undefined,
      { shallow: true }
    );
  };
  ...
  const hendleBtnTerapkan = (trapkan) => {
    ...
    router.push("/monografi-hukum/" + lctx[1] + akuksearch, undefined, {
      shallow: true,
    });
  };
              ...
              <div className="container pl-0 pr-0" id="listJns">
                {djenis &&
                  djenis.map((item) => (
                    <label
                      className="option_item"
                      key={item.jenis_monografi}
                      id={item.jenis_monografi}
                    >
                      <input
                        type="checkbox"
                        className="checkbox chckjenis"
                        name="jns[]"
                        value={item.jenis_monografi}
                      />
                      <div className="option_inner _jdih">
                        <div className="tickmark"></div>
                        <div className="name glrjns">
                          {item.jenis_monografi}
                        </div>
                      </div>
                    </label>
                  ))}
